...
import {IStrategyInterface} from "../strategies/core/interfaces/IStrategyInterface.sol";
...
contract AprOracle is Governance {
    ...
    function weightedApr(
        ...
        return
            IStrategyInterface(_strategy).totalAssets() * getStrategyApr(_strategy, 0);
    }
    ...
    function setOracle(address _strategy, address _oracle) external virtual {
        if (governance != msg.sender) {
            require(
                msg.sender == IStrategyInterface(_strategy).management(),
                "!authorized"
            );
        }
